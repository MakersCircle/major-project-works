...
class MonocularDepth:
    def __init__(self, device='cpu', checkpoint_path="./ZoeD_M12_NK.pt"):
        """
        Initializes the Monocular Depth model using a manually downloaded checkpoint.

        Args:
            device (str): The device to load the model on ('cpu' or 'cuda').
            checkpoint_path (str): Path to the manually downloaded ZoeDepth checkpoint.
        """
        model_repo = "isl-org/ZoeDepth"

        # Load model architecture without downloading weights
        depth_model = torch.hub.load(model_repo, "ZoeD_K", pretrained=False)

        # Load the checkpoint
        checkpoint = torch.load(checkpoint_path, map_location=device)

        # If checkpoint contains training-related metadata, extract only model weights
        ...
        depth_model.load_state_dict(filtered_checkpoint, strict=False)

        self.zoe_depth = depth_model.to(device).eval()
        print(f"Depth Model Loaded from {checkpoint_path} on {device}...")
